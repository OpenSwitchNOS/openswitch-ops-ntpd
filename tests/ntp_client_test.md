...
#### Test Fail Criteria
The NTP authentication key is absent from the output of the `show ntp authentication-keys` command output.
...
```
### Description
Add an NTP server using the IPV4 address and set the version as either as 3 or 4.
### Test result criteria
#### Test pass criteria
...
### Objective
Verify that the user can not add more than eight NTP servers, and an appropriate error message is shown when user tries to add more than 8 NTP servers.
### Requirements
The Virtual Mininet Test Setup is required for this test.
### Setup
#### Topology diagram
...
### Test result criteria
#### Test pass criteria
If the ntp server name is more than 15 characters, `show ntp associations` output should truncate the name to 15 characters and display.
#### Test Fail Criteria
The `show ntp associations` output doesnot truncate the server name if it is more than 15 characters and the display goes out of the table.
